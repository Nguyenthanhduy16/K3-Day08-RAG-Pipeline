...
def rerank_mmr(
    ...
    # Kiểm tra xem các candidates có embedding không, nếu không có thì fallback
    for c in candidates:
        if "embedding" not in c:
            print("Warning: candidates missing 'embedding' for MMR. Falling back to original order.")
            return candidates[:top_k]
...
def rerank(
    ...
    **kwargs
    ...
    Args:
        ...
        **kwargs: Tham số bổ sung (e.g., query_embedding cho MMR)
    ...
    if method == "cross_encoder":
        ...
        if not query_embedding:
            try:
                ...
                print(f"Failed to generate query embedding for MMR: {e}")
                return candidates[:top_k]
        return rerank_mmr(
            query_embedding, 
            candidates, 
